...
public class ServerButtonComponent extends AbstractVisualComponent {
        ...
        Runnable update = () -> {
            contextHelper.runInFxThread(() -> {
                if(currentOnline.get() == 0 && maxOnline.get() == 0) {
                    LookupHelper.<Labeled>lookup(layout, "#online").setText("сервер вимкнутий");
                } else {
    ...
    public void setOnMouseClicked(EventHandler<? super MouseEvent> eventHandler) {
        ...
        String buttonText = application.getTranslation("runtime.scenes.servermenu.serverButton.Choose-a-server");
        saveButton.setText(buttonText);

        saveButton.setOnMouseClicked(eventHandler);
    }

    public void enableSaveButton(String text, EventHandler<ActionEvent> eventHandler, boolean visible) {
